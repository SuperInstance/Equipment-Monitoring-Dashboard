...
class DashboardServer {
    ...
    constructor(activityTracker, cellVisualizer, realTimeMonitor, options = {}) {
    ...
    async start() {
        return new Promise((resolve, reject) => {
            try {
                ...
                this.wss = new ws_1.default.Server({ server: this.httpServer });
                this.setupWebSocket();
    ...
    setupRoutes() {
        ...
        // Get cells
        this.app.get('/api/cells', (_req, res) => {
            const cellData = this.cellVisualizer.toJSON();
            res.json(cellData);
        });
        // Get specific cell
        this.app.get('/api/cells/:id', (req, res) => {
        ...
        this.app.get('/api/cells/:id/provenance', (req, res) => {
        ...
        // Playback endpoints
        this.app.get('/api/playback', (_req, res) => {
            res.json({
                state: this.playbackState,
                historyLength: this.history.length,
            });
        });
        this.app.post('/api/playback/control', (req, res) => {
            const { action, value } = req.body;
            this.controlPlayback(action, value);
            res.json({ success: true, state: this.playbackState });
        });
        this.app.get('/api/playback/history', (req, res) => {
        ...
        this.app.get('/api/visualize/ascii', (_req, res) => {
            res.type('text/plain').send(this.cellVisualizer.toASCII());
        });
        // Get HTML visualization
        this.app.get('/api/visualize/html', (_req, res) => {
        ...
        switch (message.type) {
            ...
            case 'playback_control':
                if (message.payload && typeof message.payload === 'object') {
                    ...
                    this.controlPlayback(payload.action, payload.value);
                }
                break;
        }
    }
    ...
    getEquipmentStatus(equipment, _slot) {
        return {
            name: equipment.name,
            slot: equipment.slot,
            isActive: true,
    ...
    calculateAverageProcessingTime() {
        ...
        const times = activities
            ...
            .map(a => a.details.processingTime);
        if (times.length === 0)
            return 0;
